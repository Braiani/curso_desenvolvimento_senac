...
CREATE TABLE IF NOT EXISTS Professor (
    ...
    idEspecialidade int,
    dtNasc date,
    constraint foreign key fk_especialidade(idEspecialidade) references Especialidade(idEspecialidade) 
        ON DELETE CASCADE
        ON UPDATE CASCADE
);
...
-- Inserir Professores
INSERT INTO Professor (nome, idEspecialidade, dtNasc) VALUES ('Professor A', 1, '1980-01-15');
INSERT INTO Professor (nome, idEspecialidade, dtNasc) VALUES ('Professor B', 2, '1982-03-22');
INSERT INTO Professor (nome, idEspecialidade, dtNasc) VALUES ('Professor C', 3, '1978-07-30');
INSERT INTO Professor (nome, idEspecialidade, dtNasc) VALUES ('Professor D', 4, '1981-11-05');
INSERT INTO Professor (nome, idEspecialidade, dtNasc) VALUES ('Professor E', 5, '1983-09-12');
INSERT INTO Professor (nome, idEspecialidade, dtNasc) VALUES ('Professor F', 6, '1985-06-18');
...
FROM
    Professor p
JOIN
    Especialidade e ON e.idEspecialidade = p.idEspecialidade;
...
ALTER TABLE Professor ADD COLUMN idFuncao int after dtNasc;
ALTER TABLE Professor ADD FOREIGN KEY fk_funcao(idFuncao) REFERENCES Funcao(idFuncao) ON DELETE CASCADE ON UPDATE CASCADE;

-- c) Atualizar os professores inseridos e suas respectivas funções;
INSERT INTO Funcao (funcao) VALUES ('Monitor');
INSERT INTO Funcao (funcao) VALUES ('Assistente');
INSERT INTO Funcao (funcao) VALUES ('Titular');

UPDATE Professor SET idFuncao = 1 WHERE idProfessor = 1;
UPDATE Professor SET idFuncao = 2 WHERE idProfessor = 2;
UPDATE Professor SET idFuncao = 3 WHERE idProfessor = 3;
UPDATE Professor SET idFuncao = 1 WHERE idProfessor = 4;
UPDATE Professor SET idFuncao = 2 WHERE idProfessor = 5;
UPDATE Professor SET idFuncao = 3 WHERE idProfessor = 6;


-- d) Inserir um novo professor;
INSERT INTO Professor (nome, idEspecialidade, dtNasc, idFuncao) VALUES ('Professor G', 1, '1987-01-15', 3);
...
FROM
    Professor p
JOIN
    Funcao f ON f.idFuncao = p.idFuncao
WHERE f.funcao = 'Titular';

-- g) Exibir apenas as especialidades e as datas de nascimento dos professores monitores;
SELECT
    e.especialidade,
    date_format(p.dtNasc, '%d/%m/%Y') as dtNasc
FROM
    Professor p
JOIN
    Funcao f ON f.idFuncao = p.idFuncao
JOIN
    Especialidade e ON e.idEspecialidade = p.idEspecialidade
WHERE f.funcao = 'Monitor';

-- h) Atualizar a data de nascimento do idProfessor igual a 3;
UPDATE Professor SET dtNasc = '1978-07-29' WHERE idProfessor = 3;
